...
def main(args):
    ...
    for i in os.listdir(test_path):     
        ...
        # test_df = pd.read_csv('./data/evaluate/test_split/test.csv')
        test_df['reactant_id'] = test_df['reactant_id'].apply(lambda x:eval(x))
        test_df['product_id'] = test_df['product_id'].apply(lambda x:eval(x))
        evaluate(model, test_df, device, 'test', args, logger)

def evaluate(model, data_df, device, mode, args, logger):
    model.eval()
    with torch.no_grad():
        ...
        print('Match product:  %s  product_mrr: %.4f product_h1: %.4f' % (mode, product_mean_reciprocal_rank,product_h1))
        logger.info('Match product:  %s  product_mrr: %.4f product_h1: %.4f' % (mode, product_mean_reciprocal_rank,product_h1))
        ...
        for idx, data in tqdm(enumerate(zip(all_ec, all_reaction_id)), desc='enzyme mrr:'):
            ...
            map = (np.arange(1, len(match_uni)+1) / rankings).sum() / len(match_uni)
            all_enzyme_map.append(map)
        enzyme_mean_average_precision = float(np.mean(all_enzyme_map))
